...
switch (_updateType) do {
	case 1: {
		...
		sliderSetRange [_slider2, 0, _textValue min CHVD1_maxView];

		call compile format ["%1 = %2", _varType1, _textValue min CHVD1_maxView];
		call compile format ["profileNamespace setVariable ['%1',%1]", _varType1];

		if ((call compile _varType2) > _textValue) then {
			sliderSetPosition [_slider2, _textValue min CHVD1_maxObj];
			ctrlSetText [_text2, str round (_textValue min CHVD1_maxObj)];

			call compile format ["%1 = %2", _varType2, _textValue min CHVD1_maxObj];
	...
	case 2: {
		...
		sliderSetPosition [_slider1, _textValue min CHVD1_maxObj];
		//ctrlSetText [_text1, str round (_textValue min CHVD_maxObj)];

		call compile format ["%1 = %2", _varType1, _textValue min CHVD1_maxObj];
		call compile format ["profileNamespace setVariable ['%1',%1]", _varType1];
