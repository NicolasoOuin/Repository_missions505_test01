...
if (_inUAV) then {
	switch (true) do {
		case (getConnectedUAV player isKindOf "LandVehicle"): {
			if (_updateType == 1 || _updateType == 0 || _updateType == 3) then {
				setViewDistance CHVD1_car;
			};
			if (_updateType == 2 || _updateType == 0 || _updateType == 3) then {
				setObjectViewDistance [CHVD1_carObj,100];
			};
		};
		case (getConnectedUAV player isKindOf "Air"): {
			if (_updateType == 1 || _updateType == 0 || _updateType == 3) then {
				setViewDistance CHVD1_air;
			};
			if (_updateType == 2 || _updateType == 0 || _updateType == 3) then {
				setObjectViewDistance [CHVD1_airObj,100];
			};
		};
	};
} else {
	switch (true) do {
		case (vehicle player isKindOf "Man"): {
			if (_updateType == 1 || _updateType == 0 || _updateType == 3) then {
				setViewDistance CHVD1_foot;
			};
			if (_updateType == 2 || _updateType == 0 || _updateType == 3) then {
				setObjectViewDistance [CHVD1_footObj,100];
			};
		};
...
if (_updateType == 0) then {
	[_inUAV] call CHVD1_fnc_updateTerrain;
};
