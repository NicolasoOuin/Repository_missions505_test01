...
for "_x" from 1 to PARAMS_SniperTeamsPatrol do {
	...
	_randomPos = [getMarkerPos currentAO, 1000, 100, 10] call BIS_fnc_findOverwatch;
...
for "_x" from 1 to PARAMS_SniperTeamsPatrol do {
	_AAteamGroup = createGroup east;
	_randomPos = [getMarkerPos currentAO, 600, 100, 10] call BIS_fnc_findOverwatch;
	_AAteamGroup = [_randomPos, EAST,(configfile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_InfTeam_AA")] call BIS_fnc_spawnGroup;
	_AAteamGroup setBehaviour "COMBAT";
	_AAteamGroup setCombatMode "RED";

	_enemiesArray = _enemiesArray + [_AAteamGroup];

	{
		_x addCuratorEditableObjects [units _AAteamGroup, false];
	} foreach adminCurators;
...
if (PARAMS_EnemiesInBuildings != 0) then {
	...
	if ((count _town) > 6) then {
		...
		0 = [_pos,units _AOgarrisonGroup,500,0,[0,20],true,true] call SHK1_fnc_buildingPos02;
		[(units _AOgarrisonGroup)] call QS1_fnc_setSkill2;
...
[(units _patrolGroup)] call QS1_fnc_setSkill2;
[(units _overwatchGroup)] call QS1_fnc_setSkill3;
[(units _AOvehGroup)] call QS1_fnc_setSkill3;
[(units _AAteamGroup)] call QS1_fnc_setSkill2;
if (random 1 >= 0.5) then {
	[(units _sniperGroup)] call QS1_fnc_setSkill4;
} else {
	[(units _sniperGroup)] call QS1_fnc_setSkill3;
};
[(units _staticGroup)] call QS1_fnc_setSkill3;
[(units _aaGroup)] call QS1_fnc_setSkill4;

//---------- GARRISON FORTIFICATIONS

{
	_newGrp = [_x] call QS1_fnc_garrisonFortEAST;
