...
// SPAWN PATROLS
	...
	for "_counter" from 1 to _PApatrols do {
						_pos = [_mPos, _Placement, random 360] call BIS_fnc_relPos;
							_grp=[_pos,_PAgroupSize,_faction,_side] call EOS1_fnc_spawngroup;
							_aGroup set [count _aGroup,_grp];
if (_debug) then {PLAYER SIDECHAT (format ["Spawned Patrol: %1",_counter]);0= [_mkr,_counter,"patrol",getpos (leader _grp)] call EOS1_debug};
										};
	...
	for "_counter" from 1 to _LVehGroups do {
		...
		_newpos = [_mPos, (_Placement +200), random 360] call BIS_fnc_relPos;
			if (surfaceiswater _newpos) then {_vehType=8;_cargoType=10;}else{_vehType=7;_cargoType=9;};
				...
				_bGroup=[_newpos,_side,_faction,_vehType]call EOS1_fnc_spawnvehicle;

					if ((_LVgroupSize select 0) > 0) then{
							0=[(_bGroup select 0),_LVgroupSize,(_bGroup select 2),_faction,_cargoType] call eos1_fnc_setcargo;
														};

								0=[(_bGroup select 2),"LIGskill"] call eos1_fnc_grouphandlers;
								_bGrp set [count _bGrp,_bGroup];
if (_debug) then {player sidechat format ["Light Vehicle:%1 - r%2",_counter,_LVehGroups];0= [_mkr,_counter,"Light Veh",(getpos leader (_bGroup select 2))] call EOS1_debug};
		};
...
//SPAWN ARMOURED VEHICLES
	...
	for "_counter" from 1 to _AVehGroups do {
					...
					_cGroup=[_newpos,_side,_faction,_vehType]call EOS1_fnc_spawnvehicle;

						0=[(_cGroup select 2),"ARMskill"] call eos1_fnc_grouphandlers;
					_cGrp set [count _cGrp,_cGroup];
if (_debug) then {player sidechat format ["Armoured:%1 - r%2",_counter,_AVehGroups];0= [_mkr,_counter,"Armour",(getpos leader (_cGroup select 2))] call EOS1_debug};
		};
...
//SPAWN HELICOPTERS
	...
	if ((_fSize select 0) > 0) then {_vehType=4}else{_vehType=3};
				_newpos = [(markerpos _mkr), 1500, random 360] call BIS_fnc_relPos;
						_fGroup=[_newpos,_side,_faction,_vehType,"fly"]call EOS1_fnc_spawnvehicle;
						_CHside=_side;
						_fGrp set [count _fGrp,_fGroup];

	if ((_fSize select 0) > 0) then {
		_cargoGrp = createGroup _side;
			0=[(_fGroup select 0),_fSize,_cargoGrp,_faction,9] call eos1_fnc_setcargo;
			0=[_cargoGrp,"INFskill"] call eos1_fnc_grouphandlers;

				_fGroup set [count _fGroup,_cargoGrp];
				null = [_mkr,_fGroup,_counter] execvm "eos1\functions1\TransportUnload1_fnc.sqf";
						...
						*/
							...
							if (_debug) then {player sidechat format ["Chopper:%1",_counter];0= [_mkr,_counter,"Chopper",(getpos leader (_fGroup select 2))] call EOS1_debug};
			};
	...
	sleep 1;
			if (!triggeractivated _bastActive || getmarkercolor _mkr == "colorblack") exitwith
			{
					...
					_mkr setmarkeralpha _mAN;
						...
						if (_eosZone) then {
null = [_mkr,[_PApatrols,_PAgroupSize],[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups,0,0,0],[_faction,_mA,350,_CHside]] execVM "eos1\core1\EOS1_Core.sqf";
...
null = [_mkr,[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups],[_CHGroups,_fSize],_settings,[_pause,_waves,_timeout,_eosZone,_hints],true] execVM "eos1\core1\b1_core.sqf";
						};};
